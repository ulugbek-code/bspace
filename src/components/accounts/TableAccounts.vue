<template>
    <div class="container">
        <table>
            ...
            <div v-if="getLoader" id="loader"><img src="../../assets/loader.gif" alt=""></div>
            <div v-else-if="!getLoader && getError">{{getError}}</div>
            <div v-else-if="!getLoader && (!getAccountsList || getAccountsList.length === 0)">
                No stored firms yet or <br> Authorization problem
            </div>
            <tr v-for="content in filteredContents" :key="content">
                <td>
                    <span id="code" :style="{backgroundColor: `hsla(${~~(360 * Math.random())},70%,70%,0.8)`}">
                        {{content.code}}
                    </span>
                </td>
                <td>{{content.name}}</td>
                <td :class="content.isActive ? 'blue-bg' : 'red-bg'">{{content.isActive ? 'Active' : 'Passive'}}</td>
                <td id="category" >
                    <span v-for="(c,i) in content.categories" :key="i">
                        {{c.name}}{{++i !== content.categories.length ? ',' : ''}} <br>
                    </span>
                </td>
                ...
                <td>
                    <template v-for="(types,i) in content.ifrsCategories" :key="i">
                        <template v-for="t in types.types" :key="t">{{t}}{{++i !== types.types.length ? ', ' : ' '}}</template>
                    </template>
                </td>
                <div id="flex-div">
                    <div id="edit-trash">
                        <fa @click="editItem(content.id)" class="icons icon-edit" :icon="['fas', 'edit']" />
                        <fa @click="deleteItem(content.id)" class="icons icon-trash" :icon="['fas', 'trash']" />
                    </div>
                </div>
            </tr>
        </table>
...
export default {
    ...
    data(){
        return{
            titles: ['CODE', 'NAME', 'TYPE', 'ACC CATEGORY', 'ACC N', 'IFRS KMPG', 'TYPES'],
            categoriesName: [],
            ifrsCategories: [],
            ifrsTypes: [],
            parentAccounts: [],
    ...
    computed:{
        filteredContents(){
            return this.getAccountsList.filter((content)=>{
                return content.name.match(this.search) || content.code.match(this.search) 
                || content.ifrsCategories[0].code.match(this.search) || content.ifrsCategories[0].name.match(this.search)
            })
        },
        getAccountsList(){
    ...
    methods:{
        deleteItem(id){
            if(confirm("do you want to delete?")){
                axios.delete("https://bspacedev.azurewebsites.net/api/Accounts/Delete/"+id,
                {
                    headers: {
        ...
        async editItem(id){
            await axios.get("https://bspacedev.azurewebsites.net/api/Accounts/GetById/"+id,
            {
                    headers: {
                        "Authorization" : `Bearer ${localStorage.getItem("mytoken")}`
                    }
